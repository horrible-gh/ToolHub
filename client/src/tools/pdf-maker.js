...
const ALLOWED = new Set(['docx', 'pptx', 'md', 'markdown']);
...
export function validatePdfMakerFiles(files,currentCount=0){const accepted=[],rejected=[];for(const file of files){const ext=file.name.split('.').pop()?.toLowerCase()||'';if(!ALLOWED.has(ext))rejected.push(file.name+': only DOCX, PPTX, and Markdown (.md/.markdown) files are supported.');else if(file.size>MAX_FILE_BYTES)rejected.push(file.name+': file exceeds 25 MB.');else if(currentCount+accepted.length>=MAX_FILES)rejected.push(file.name+': only 10 files can be converted at once.');else accepted.push(file);}return{accepted,rejected};}
const errors={UNSUPPORTED_FORMAT:'This file format is not supported.',FILE_TOO_LARGE:'This file is larger than the server limit.',INVALID_DOCUMENT:'This file is damaged or does not match its file type.',ENCRYPTED_DOCUMENT:'Password-protected documents cannot be converted.',CONVERSION_TIMEOUT:'Conversion took too long. Try a smaller document.',CONVERTER_FAILED:'The document could not be converted.',INVALID_PDF:'The converter did not produce a valid PDF.'};
const statusText=(file)=>file.error?(errors[file.error.code]||file.error.message||'Conversion failed.'):({selected:'Ready',uploading:'Uploading',queued:'Queued',converting:'Converting',succeeded:'Ready to download',failed:'Failed',expired:'Expired'}[file.status]||file.status);
