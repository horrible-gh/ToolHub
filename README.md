...
Configuration is fail-closed. Supported variables are `PDF_MAKER_ALLOWED_EXTENSIONS` (must include `.docx,.pptx,.md,.markdown`), `PDF_MAKER_FILE_MAX_BYTES`, `PDF_MAKER_REQUEST_MAX_BYTES`, `PDF_MAKER_MAX_FILES`, `PDF_MAKER_TIMEOUT_MS`, `PDF_MAKER_CONCURRENCY`, `PDF_MAKER_QUEUE_LIMIT`, `PDF_MAKER_RETENTION_MS`, `PDF_MAKER_CLEANUP_INTERVAL_MS`, `PDF_MAKER_STORAGE_ROOT`, and `PDF_MAKER_LIBREOFFICE_PATH`. Invalid values stop startup with `INVALID_ENV`. Shutdown stops intake, waits for active conversions, and stops cleanup scheduling; retained jobs are removed by expiry after restart only when durable job metadata is introduced, so current deployments must use a single process and clear an abandoned isolation root operationally before restart.

`.md`/`.markdown` uploads are validated separately from DOCX/PPTX: the raw bytes must decode as strict UTF-8, contain no NUL byte, and not start with a ZIP/OOXML, legacy CFB/OLE, or PDF magic prefix (binary files masquerading with a Markdown extension are rejected as `INVALID_DOCUMENT`). Accepted Markdown is rendered with the shared no-network renderer (`shared/markdown-renderer.js`, `PDF` resource policy) into a self-contained standalone HTML document, staged into the per-conversion runtime directory as `i.html`, and converted with the LibreOffice headless pipeline -- never with `PDF_MAKER_ENGINE=office`'s PowerShell/Office COM path, even when that engine is configured for DOCX/PPTX in the same deployment. The queue, timeout, process isolation, output containment, storage, download, ZIP, and cleanup contracts described above apply identically to Markdown conversions.

## MD Viewer

MD Viewer reads and renders `.md`/`.markdown` files entirely in the browser: nothing is uploaded, no server endpoint is involved, and no external CDN is loaded. A single file up to 5 MB is read with `File.text()`, converted with the bundled `marked` parser, and sanitized with the bundled `DOMPurify` before insertion into the page. Relative links/images and unsupported URL schemes are shown as inert placeholders instead of being requested, and external links open with `rel="noopener noreferrer"`. Choosing a new file, or removing the current one, fully replaces any previous rendered output, error, and copy-button state. This in-browser path is unrelated to PDF-Maker's server-side Markdown-to-PDF conversion above: MD Viewer never uploads a file, and PDF-Maker never renders in the browser.